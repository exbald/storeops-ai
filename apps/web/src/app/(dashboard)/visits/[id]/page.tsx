...
export default function VisitDetailPage() {
  ...
  const handleTriggerInvestigation = async (e: React.FormEvent) => {
    ...
    if (uploadedMedia.length === 0) {
      setInvError("Please upload at least one shelf photo before triggering investigation.");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-5">
        ...
        <div className="flex gap-3">
          ...
          {visit.status === "OPEN" && (
            <Button onClick={() => setIsInvestigateOpen(true)}>
        ...
        {uploadedMedia.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
            {uploadedMedia.map((media) => (
              <div key={media.id} className="border border-gray-200 rounded p-3 text-xs space-y-1">
          ...
          <p className="text-sm text-gray-400 italic">No media uploaded yet for this session.</p>
        )}
      </div>
      ...
      >
        <form onSubmit={handleTriggerInvestigation} className="space-y-4">
          ...
          <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
            <strong>Photos included:</strong> {uploadedMedia.length} photo(s) will be submitted as visual evidence.
          </div>
